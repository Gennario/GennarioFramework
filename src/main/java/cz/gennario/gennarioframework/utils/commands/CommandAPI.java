...
public class CommandAPI {
    ...
    public void buildCommand() {
        try {
            if (help) {
                SubCommand subCommand1 = addCommand("help")
                        ...
                        .setResponse((sender, label, commandArgs) -> {
                            for (SubCommand subCommand : subCommands) {
                                sender.sendMessage("§8- §7/" + label + " " + subCommand.getUsage() + " §8- §f" + subCommand.getDescription());
                            }
                        });
                if (permission != null && !permission.isEmpty()) {
                    subCommand1.setPermission(permission);
                }
            }
            ...
            pluginCommand.setExecutor((sender, cmd, label, args) -> {
                if (args.length == 0) {
                    if (permission != null && !permission.isEmpty()) {
                        ...
                        if (!hasPermission) {
                            sender.sendMessage(Utils.colorize(null, commandSettings.getNoPermission()));
                            return false;
                        }
                    }
                    if (help && emptyCommandResponse == null) {
                        String replace = new ReplacementPackage()
                                .append("%label%", label)
                                .append("%help%", "help [page]")
                                .replace(Utils.colorize(commandSettings.getUsage()));
                        ...
                        if (command.getCommand().equalsIgnoreCase(s) || command.getAliases().contains(s.toLowerCase())) {
                            ...
                            for (SubCommandArg commandArg : command.getSubCommandArgs()) {
                                if (i + 1 > commandArgs.size()) {
                                    ...
                                    if (!correctType) {
                                        String replace = new ReplacementPackage()
                                                .append("%value%", commandArg.getName())
                                                .replace(Utils.colorize(commandSettings.getWrongUsage()));
                                        sender.sendMessage(replace);
                                ...
                                switch (subCommandArg.getValue()) {
                                    ...
                                    case LOCATION:
                                        if(sender instanceof Player player) {
                                            list.add(LocationUtils.locationToString(player.getLocation().getBlock().getLocation()));
                                        }
                                        return list;
                                }
                            }
